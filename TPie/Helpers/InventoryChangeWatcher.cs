...
public class InventoryChangeWatcher
{
    ...
    {
        // Sigs valid as of 6.5

        if (Plugin.SigScanner.TryScanText("E8 ?? ?? ?? ?? 4C 8B 0D ?? ?? ?? ?? 41 B2 01", out var setKeyItemPtr))
        {
            SetKeyItemHook = Plugin.GameInteropProvider.HookFromAddress<SetKeyItem>(setKeyItemPtr, SetKeyItemDetour);
            SetKeyItemHook.Enable();
        }
        else
        {
            Plugin.Logger.Warning("Failed to hook SetKeyItem!");
            LoadFailure();
        ...
        {
            UnsetKeyItemHook = Plugin.GameInteropProvider.HookFromAddress<UnsetKeyItem>(unsetKeyItemPtr, UnsetKeyItemDetour);
            UnsetKeyItemHook.Enable();
        }
        else
        {
            Plugin.Logger.Warning("Failed to hook UnsetKeyItem!");
            LoadFailure();
        ...
        {
            ModifyKeyItemHook = Plugin.GameInteropProvider.HookFromAddress<ModifyKeyItem>(modifyKeyItemPtr, ModifyKeyItemDetour);
            ModifyKeyItemHook.Enable();
        }
        else
        {
            Plugin.Logger.Warning("Failed to hook ModifyKeyItemHook!");
            LoadFailure();
    ...
    protected void Dispose(bool disposing)
    {
        ...
        SetKeyItemHook?.Dispose();
        UnsetKeyItemHook?.Dispose();
        ModifyKeyItemHook?.Dispose();
    ...
    {
        Plugin.Logger.Warning("Unable to initialize inventory change watcher. Inventory will be checked on each frame update.");
    ...
    private void GameNetworkOnNetworkMessage(IntPtr dataPtr, ushort opCode, uint sourceActorId, uint targetActorId, NetworkMessageDirection direction)
    {
        if (_loadState == LoadState.Finished)
        {
            ...
            Plugin.Logger.Debug($"@@@ {OpcodeNames[index]} ({OpcodeValues[index]}) :: {sourceActorId}, {targetActorId}, {direction}");
            InventoryChanged();
        }
    }
    ...
    private void SetKeyItemDetour(IntPtr agent, ushort slotId, int itemId)
    {
        if (_loadState == LoadState.Finished)
        {
            Plugin.Logger.Debug($"= SetKeyItemDetour({slotId}, {itemId})");
            InventoryChanged();
        }
    ...
    private void UnsetKeyItemDetour(IntPtr agent, ushort slotId)
    {
        if (_loadState == LoadState.Finished)
        {
            Plugin.Logger.Debug($"= UnsetKeyItemDetour({slotId})");
            InventoryChanged();
        }
    ...
    private char ModifyKeyItemDetour(IntPtr agent)
    {
        if (_loadState == LoadState.Finished)
        {
            Plugin.Logger.Debug($"= ModifyKeyItemDetour()");
            InventoryChanged();
        }
